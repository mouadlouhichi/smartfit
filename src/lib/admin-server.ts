...
export async function requirePlatformAdmin(req: Request): Promise<PlatformAdminGuard> {
  ...
  try {
    ...
    if (
      account.disabled ||
      account.customClaims?.[PLATFORM_ROLE_CLAIM] !== PLATFORM_ADMIN_CLAIM_VALUE
    ) {
  ...
  const summaries = await Promise.all(
    gymsSnap.docs.map(async (doc) => {
      ...
      for (const m of members.docs) {
        ...
        if (isGymCustomer({ role })) memberCount++;
        else staffCount++;
      }
      ...
      const branding = (data.branding as GymBranding | undefined) ?? undefined;
      const contact =
