 ...
 */
import { demoFixture } from './tenant-demo';
import { DEFAULT_TENANT_PLANS, isGymCustomer } from '@smartfit/core';
...
export function demoAdminGyms(): AdminGymSummary[] {
  return [
    ...
    return fixture
      ? ({
          ...gym,
          memberCount: fixture.roster.filter(isGymCustomer).length,
          staffCount: fixture.roster.filter((m) => !isGymCustomer(m)).length,
          classCount: fixture.classes.length,
        } as AdminGymSummary)
      : (gym as AdminGymSummary);
  });
}
