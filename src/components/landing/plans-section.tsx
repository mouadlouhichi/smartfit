    ...
    <section id="plans" ref={ref} className="relative overflow-x-clip py-24 lg:py-32">
      <div className="mx-auto max-w-[1400px] px-6 lg:px-12">
        <div className="mb-16 max-w-3xl lg:mb-20">
          ...
          <h2 className="reveal text-4xl tracking-tight lg:text-6xl" data-state={visible ? 'visible' : 'hidden'}>
            Four proven splits.
            <br />
            <span className="text-[color:var(--muted-foreground)]">Pick the one that fits your week.</span>
          </h2>
          ...
          {PLANS.map((plan, index) => (
            ...
            >
              <span className="font-mono text-xs text-[color:var(--muted-foreground)]">
                {new Intl.NumberFormat('en-US', { minimumIntegerDigits: 2, useGrouping: false }).format(index + 1)}
              </span>
              <h3 className="mt-4 font-display text-2xl leading-tight">{plan.name}</h3>
              <p className="mt-3 text-sm leading-relaxed text-[color:var(--muted-foreground)]">{plan.description}</p>
