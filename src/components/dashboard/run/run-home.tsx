...
import { timeOfDayPeriod } from '@/lib/run-sensors';
...
export function RunHome({ runs, onStart }: { runs: WorkoutSession[]; onStart: () => void }) {
  ...
  const days = useMemo(() => weekBars(trackedState, runs, locale), [trackedState, runs, locale]);
  ...
  return (
    <div className="card-hero relative overflow-hidden p-5 text-white sm:p-7">
      <span
        aria-hidden
        className="pointer-events-none absolute -top-24 -right-16 h-64 w-64 rounded-full bg-[radial-gradient(circle,rgba(138,210,0,0.5),transparent_70%)]"
      />
      <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,22rem)] lg:items-center">
        ...
        <div className="grid gap-4">
          <p className="hero-muted flex flex-wrap items-center gap-2 text-sm font-bold">
            {firstName
              ? `${t(`run.greeting.${timeOfDayPeriod()}`)}, ${firstName}`
              : t(`run.greeting.${timeOfDayPeriod()}`)}
            {streak > 0 && (
          ...
          >
            ...
            <span className="flex-1 text-base font-extrabold">{t('run.home.start')}</span>
            <ChevronsRight
          ...
          <div className="hero-muted flex flex-wrap items-center gap-2 text-[11px] font-bold">
            <span className="hero-tile inline-flex items-center gap-1.5 rounded-full px-2.5 py-1">
              <Gauge className="h-3 w-3" aria-hidden /> {t('run.home.chip.splits')}
            </span>
            <span className="hero-tile inline-flex items-center gap-1.5 rounded-full px-2.5 py-1">
              <Mountain className="h-3 w-3" aria-hidden /> {t('run.home.chip.elevation')}
            </span>
            <span className="hero-tile inline-flex items-center gap-1.5 rounded-full px-2.5 py-1">
              <Share2 className="h-3 w-3" aria-hidden /> {t('run.home.chip.share')}
            </span>
          </div>
        </div>
        ...
        <div className="grid gap-3">
          <div className="from-volt to-volt-dim text-ink relative overflow-hidden rounded-3xl bg-gradient-to-br p-5 shadow-lg">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-black/10 px-2.5 py-1 text-[11px] font-bold">
              <Footprints className="h-3 w-3" aria-hidden /> {t('run.home.week.label')}
            </span>
            ...
            <p className="text-ink/85 mt-3 flex items-center gap-1.5 text-xs font-bold">
              {deltaPct === null ? (
                week.distance > 0 ? (
                  t('run.home.week.first')
                ) : (
                  t('run.home.week.none')
                )
              ) : (
                <>
                  {deltaPct >= 0 ? (
                  ...
                  {t(deltaPct >= 0 ? 'run.home.week.deltaUp' : 'run.home.week.deltaDown', {
                    percent: Math.abs(deltaPct).toFixed(1),
                    distance: lastWeek.distance.toFixed(1),
                  })}
                </>
              )}
            </p>
...
function weekBars(
  state: Parameters<typeof thisWeek>[0],
  runs: WorkoutSession[],
  locale?: string,
): DayBar[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    out.push({
      iso,
      label: weekdayLabels(locale, 'short')[d.getDay()],
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/25 p-4 backdrop-blur-sm">
      <div className="flex items-center justify-between gap-2">
        <p className="eyebrow text-white/60">{t('run.home.week.days')}</p>
      ...
      <ul className="sr-only" aria-label={t('run.home.week.daysAria')}>
        {days.map((d) => (
          <li key={`${d.iso}-text`}>
            {t('run.home.week.dayDetail', {
              day: d.label,
              km: d.distanceKm.toFixed(2),
              metres: Math.round(d.elevationGainM),
            })}
          </li>
        ))}
      </ul>
      {weekKm === 0 && <p className="mt-2 text-[11px] text-white/55">{t('run.home.week.empty')}</p>}
    </div>
  );
}
...
export function RunnerMark({ className = '' }: { className?: string }) {
  const { t } = useI18n();
  return (
    <svg viewBox="0 0 100 100" className={className} role="img" aria-label={t('run.markAria')}>
      <g
        fill="none"
        stroke="currentColor"
        strokeWidth={8}
