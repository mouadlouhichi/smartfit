...
const STYLES: { id: ShareStyle; labelKey: string; hintKey: string }[] = [
  {
    id: 'transparent',
    labelKey: 'share.style.transparent',
    hintKey: 'share.style.transparentHint',
  },
  { id: 'dark', labelKey: 'share.style.volt', hintKey: 'share.style.voltHint' },
  { id: 'light', labelKey: 'share.style.ink', hintKey: 'share.style.inkHint' },
];

const FORMATS: { id: ShareFormat; labelKey: string; ratio: string }[] = [
  { id: 'square', labelKey: 'share.format.post', ratio: '4:5' },
  { id: 'story', labelKey: 'share.format.story', ratio: '9:16' },
];
  ...
  async function run(action: 'share' | 'download' | 'copy') {
    ...
    try {
      if (action === 'share') {
        ...
        toast(result === 'shared' ? t('share.shared') : t('share.saved'));
      } else if (action === 'download') {
        await shareOrDownload(blob, filename, `${data.title} · SmartFit`);
        toast(t('share.saved'));
      } else {
        const ok = await copyPng(blob);
        toast(ok ? t('share.copied') : t('share.copyUnsupported'), ok ? undefined : 'info');
      }
    } catch {
      toast(t('share.failed'), 'info');
    } finally {
      setBusy(null);
    }
  }
        ...
        <div className="mt-4 grid gap-5 sm:grid-cols-[minmax(0,16rem)_minmax(0,1fr)]">
            ...
            >
              ...
              {previewing && previewUrl && (
                <span className="bg-background/80 text-foreground absolute right-4 bottom-4 flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold">
                  <Loader2 className="h-3 w-3 animate-spin" aria-hidden /> {t('share.rendering')}
                </span>
              )}
            </div>
            <p className="text-muted-foreground text-center text-[11px]">
              {transparent
                ? t('share.hint.transparent')
                : format === 'story'
                  ? t('share.hint.story')
                  : t('share.hint.feed')}
            </p>
          ...
          <div className="grid content-start gap-4">
            <fieldset className="grid gap-2">
              <legend className="text-foreground/90 text-sm font-medium">{t('share.style')}</legend>
              <div className="grid gap-2">
                {STYLES.map((option) => (
                  <button
                    key={option.id}
            ...
            <div className="grid gap-2">
              <p className="text-foreground/90 text-sm font-medium">{t('share.frame')}</p>
